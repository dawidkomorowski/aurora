...
internal static class TestKit
{
    ...
    public static void AssertThatIssueDetailsResponsesAreEqual(IssueDetailsResponse actual, IssueDetailsResponse expected)
    {
    ...
    public static void AssertThatVersionDetailsResponsesAreEqual(VersionDetailsResponse actual, VersionDetailsResponse expected)
    {
        Assert.That(actual.Id, Is.EqualTo(expected.Id));
    ...
    public static async Task<CreateChecklistResponse> CreateChecklist(HttpClient client, int issueId, string title)
    {
        ...
        await VerboseEnsureSuccessStatusCode(response);
        return await response.Content.ReadFromJsonAsync<CreateChecklistResponse>() ?? throw UnexpectedContent();
    }

    public static async Task CreateChecklistItem(HttpClient client, int checklistId, string content)
    {
        var createChecklistItemRequest = new CreateChecklistItemRequest
        {
            Content = content
        };
        using var contentJson = CreateJsonContent(createChecklistItemRequest);
        using var response = await client.PostAsync($"api/checklists/{checklistId}/items", contentJson);
        await VerboseEnsureSuccessStatusCode(response);
    }
    ...
    public static async Task<ChecklistResponse[]> GetAllChecklists(HttpClient client, int issueId)
    {
        ...
        await VerboseEnsureSuccessStatusCode(response);
        return await response.Content.ReadFromJsonAsync<ChecklistResponse[]>() ?? throw UnexpectedContent();
    }

    public static async Task<ChecklistResponse> GetChecklist(HttpClient client, int checklistId)
    {
        using var response = await client.GetAsync($"api/checklists/{checklistId}");
        await VerboseEnsureSuccessStatusCode(response);
        return await response.Content.ReadFromJsonAsync<ChecklistResponse>() ?? throw UnexpectedContent();
    }
